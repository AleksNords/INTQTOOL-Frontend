...
export default function AdminToolsNavigator({setCurrentTool}) {
    ...
    return (<div className={"admin-tools-navigator-wrapper"}>
        <div className={"admin-tools-navigator"}>
            <h1 id="user-tools-headers" onClick={(elem) => toggleActiveArchiveQuiz(elem)}
                className={"quiz-type-navigator-title quiz-type-navigator-title-enabled"}>User tools</h1>
            <h1 id="course-tools-headers" onClick={(elem) => toggleActiveArchiveQuiz(elem)}
                className={"quiz-type-navigator-title quiz-type-navigator-title-disabled"}>Course tools</h1>
