    ...
    return (
            <div className="quiz-results-modulo">
                            ...
                            <Grid container spacing={1}>
                                {resultArray.map((questionAns, index) => (
                                    <Grid key={questionAns} item>
                                        <Box onClick={(index) => (handleQuestionClick(index))} className={"grid-elem " + getQuestionGradeColor(questionAns)}>{index + 1}</Box>
                                    </Grid>
                                ))}
                            </Grid>
                        </Grid>
                ...
                <Box className="progress-box" sx={{ position: 'relative', display: 'inline-flex' }}>
                    ...
                    >
                        <Typography fontSize={"8rem"} fontWeight={"bold"} variant="caption" component="div" color="text.secondary">
                            {`${result}%`}
                        </Typography>
                    </Box>
                </Box>
